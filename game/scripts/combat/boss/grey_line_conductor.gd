...
func _update_phase() -> void:
	...
	if hp_percent <= 0.3:
		...
		if not enraged:
			...
			visual.color = Color(0.6, 0.1, 0.1, 1)
	elif hp_percent <= 0.6:
		phase = 2
...
func _ticket_punch() -> void:
	...
	tw.tween_callback(func():
		...
		dash_tw.tween_property(self, "global_position", charge_pos + dash_dir * 300, 0.25)
...
func _summon_passengers() -> void:
	...
	for i in range(count):
		...
		enemy.move_speed = data.get("move_speed", 150.0)
		enemy.damage = data.get("damage", 15)
		...
		enemy.global_position = global_position + Vector2(randf_range(-120, 120), randf_range(-120, 120))
...
func _broadcast_static() -> void:
	...
	tw.tween_callback(func():
		...
		if player.global_position.distance_to(global_position) <= radius:
			...
			var slow_timer: SceneTreeTimer = get_tree().create_timer(2.0)
			slow_timer.timeout.connect(func(): player.move_speed = original_speed)
	)
...
func take_damage(amount: int) -> void:
	...
	flash_tw.tween_property(visual, "modulate", Color.WHITE, 0.08)
	flash_tw.tween_property(visual, "modulate", Color(0.4, 0.4, 0.4, 1), 0.08)
	if current_hp <= 0:
		_die()
